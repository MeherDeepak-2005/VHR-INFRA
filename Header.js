...
function HeaderComponent({ styles }) {
  ...
  return (
    <Box position='relative' w='full' className={styles.header}>
      ...
      <Button onClick={()=>{handleRoute('/projects')}} m='10' px={['5','10']} py='8' bg='yellow.200' borderRadius='2xl' size='lg' _hover={{ backgroundColor:'yellow.300',transform: 'scale(1.05)' }}>
      ...
      <Box flexDirection={['row', 'row', 'column']} display={['flex', 'flex', 'flex']} position='absolute' top={[0, 0, 'auto']} right={['.5rem', '1.5rem', '2rem']} w={['20rem', '20rem', '4rem']} h={['4rem', '4rem', '20rem']} className={styles.navigation}>
        ...
        <svg style={{cursor: 'pointer'}} onClick={() => { handleRoute('/projects')}} fill="#000000" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" width="24px" height="24px"><path d="M22 20L20 22 14 16 14 14 16 14z"/><path fill="none" stroke="#000000" strokeMiterlimit="10" strokeWidth="2" d="M9 3A6 6 0 1 0 9 15A6 6 0 1 0 9 3Z"/><path fill="none" stroke="#000000" strokeMiterlimit="10" d="M13 13L15.5 15.5"/></svg>
          <Image onClick={
            () => {
