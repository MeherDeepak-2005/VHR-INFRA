...
function Projects({ fetchedProjects }) {
  ...
  const [projects, setProjects] = useState(JSON.parse(fetchedProjects));
  ...
  if (searchStatus) {
      ...
      <>
        ...
        <Flex direction='row' flexWrap='wrap'>
          {projects.map(project => (
            <Link key={project.data().id} passHref href={project.data().file}>
              <Box overflow='hidden' bg='gray.50' w='40rem' m='1rem' py={10} px={10}>
                <Image w='40rem' h='20rem' transition='all .5s ease-in' _hover={{ transform: 'scale(1.1)' }} borderRadius={10} src={project.data().image} alt="VHR Infra's Project" />
                <VStack>
    ...
    return (
      <>
        ...
        <Flex direction='row' flexWrap='wrap'>
          {projects.map(project => (
            <Link key={project.id} passHref href={project.file || 'http://Googleapis.com'}>
              <Box overflow='hidden' bg='gray.50' w='40rem' m='1rem' py={10} px={10}>
